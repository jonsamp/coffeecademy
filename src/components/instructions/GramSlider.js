...
const GramSlider = ({
  ...
  return (
    <div className={styles.gramSlider}>
      <h1>{currentGrams}</h1>
      <h3>GRAMS</h3>
      <Slider min={min} max={max} onChange={setGrams} tipFormatter={cupFormatter} tipTransitionName="rc-slider-tooltip-zoom-down" included={true} className={styles.slider}/>
    </div>
  )
}
